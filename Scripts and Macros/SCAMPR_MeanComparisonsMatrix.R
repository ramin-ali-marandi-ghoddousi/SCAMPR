####---------- SCAMPR_MeanComparisonsMatrix ---------#####

##### Use this script to compare the per-animal mean expression of genes from Hiplex RNAscope between two experimental groups
...
Means[,y_min := min(Means*0.75), by = Gene]
Means[,y_max := max(Means*1.25), by = Gene]
head(Means)

## Plot means as boxplots!
ggplot(Means, aes(x = Group, y = Means, fill = Group))+
  ...
  stat_compare_means(method = statTest, label.y.npc = .9, size = 4.5)+
  theme_bw()+
...
## Calculate mean of means for each boxplot
mean.of.Means <- Means %>%
  group_by(Group, Gene) %>%
  summarize(Means = mean(Means))

print(mean.of.Means, n=nrow(mean.of.Means))
